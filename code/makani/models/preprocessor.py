...
class Preprocessor2D(nn.Module):
    ...
    def cache_unpredicted_features(self, x, y, xz=None, yz=None, xsw=None, ysw=None):
        """Cache zenith angle and/or space weather as unpredicted features"""
        
        # Combine zenith and space weather if both present
        if xz is not None and xsw is not None:
            xz_combined = torch.cat([xz, xsw], dim=2)
        elif xsw is not None:
            xz_combined = xsw
        else:
            xz_combined = xz
        
        if yz is not None and ysw is not None:
            yz_combined = torch.cat([yz, ysw], dim=2)
        elif ysw is not None:
            yz_combined = ysw
        else:
            yz_combined = yz
        ...
        if self.training:
            if (self.unpredicted_inp_train is not None) and (xz_combined is not None):
                self.unpredicted_inp_train.copy_(xz_combined)
            else:
                self.unpredicted_inp_train = xz_combined

            if (self.unpredicted_tar_train is not None) and (yz_combined is not None):
                self.unpredicted_tar_train.copy_(yz_combined)
            else:
                self.unpredicted_tar_train = yz_combined
        else:
            if (self.unpredicted_inp_eval is not None) and (xz_combined is not None):
                self.unpredicted_inp_eval.copy_(xz_combined)
            else:
                self.unpredicted_inp_eval = xz_combined

            if (self.unpredicted_tar_eval is not None) and (yz_combined is not None):
                self.unpredicted_tar_eval.copy_(yz_combined)
            else:
                self.unpredicted_tar_eval = yz_combined
