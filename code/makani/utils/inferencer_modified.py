...
class Inferencer(Trainer):
    ...
    def _autoregressive_inference(self, data, compute_metrics=False, output_data=False, output_channels=[0, 1],output_fig=False):
        # map to gpu
        ...
        for idt, targ in enumerate(tarlist):
            ...
            if output_data:
                ...
                pred_denorm_expanded = np.expand_dims(pred_denorm, axis=0)  # Shape becomes [16, 1, 144, 192, 288]
                targ_denorm_expanded = np.expand_dims(targ_denorm, axis=0)
                print(pred_denorm_expanded.shape)
                # Append along the new axis
                self.pred_outputs.append(pred_denorm_expanded)
                self.pred_outputs.append(targ_denorm_expanded)
        ...
        if output_fig:
            print(f"the prediction output shape is {self.pred_output_concat.shape}")
            visualize.plot_inference_comparison(self.pred_output_concat, self.targ_output_concat, self.params, self.lat, self.lon, cmap="twilight_shifted", projection="mollweide", diverging=False, figsize=(20, 15), vmax=None, title_str=None)
        # append history
        return 

    def inference_single(self, data, compute_metrics=False, output_data=False, output_channels=[0, 1], output_fig=False):
        """
        ...
        with torch.inference_mode():
            with torch.no_grad():
                ...
                self._autoregressive_inference(data, compute_metrics=compute_metrics, output_data=output_data, output_channels=output_channels,output_fig=output_fig)
        ...
        with torch.inference_mode():
            with torch.no_grad():
                ...
                for data in tqdm(self.valid_dataloader, desc="Scoring progress", disable=not self.params.log_to_screen):
                    #if eval_steps == 0:
                    #    self.inference_single(data, compute_metrics=False, output_data=False, output_channels=False, output_fig=True)
                    eval_steps += 1
                    self._autoregressive_inference(data, compute_metrics=True, output_data=True, output_channels=False,output_fig=False)

        # create final logs
        logs, acc_curves, rmse_curves = self.metrics.finalize(final_inference=True)

        # save the acc curve, pred_data, and target data
        if self.world_rank == 0:
            ...
            np.save(os.path.join(self.params.experiment_dir, "pred_outputs.npy"), np.concatenate(self.pred_outputs, axis=0).cpu().numpy())
            np.save(os.path.join(self.params.experiment_dir, "targ_outputs.npy"), np.concatenate(self.targ_outputs, axis=0).cpu().numpy())
