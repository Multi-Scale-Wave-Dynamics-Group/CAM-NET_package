...
class GeneralES(object):
    ...
    def _get_stats_h5(self, enable_logging):
        with h5py.File(self.files_paths[0], "r") as _f:
            ...
            if "time" in _f:
                self.time_stamp = np.array(_f["time"][:])
                logging.info("Found time dataset with {} entries.".format(self.time_stamp.shape[0]))
            else:
                self.has_time = False
                self.time_stamp = None
                logging.info("No time dataset found in the file.")
