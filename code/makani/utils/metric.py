...
class MetricsHandler:
    ...
    def __init__(
        ...
        clim,
        device,
        rmse_var_names=["U_level_90km","V_level_90km","WI_level_90km","T_level_90km","U_level_100km","V_level_100km","WI_level_100km","T_level_100km","U_level_110km","V_level_110km","WI_level_110km","T_level_110km"],
        acc_vars_names=["U_level_90km","V_level_90km","WI_level_90km","T_level_90km","U_level_100km","V_level_100km","WI_level_100km","T_level_100km","U_level_110km","V_level_110km","WI_level_110km","T_level_110km"],
        acc_auc_var_names=["U_level_90km","V_level_90km","WI_level_90km","T_level_90km","U_level_100km","V_level_100km","WI_level_100km","T_level_100km","U_level_110km","V_level_110km","WI_level_110km","T_level_110km"]
    ):
        self.device = device
        self.log_to_screen = params.log_to_screen
        self.log_to_wandb = params.log_to_wandb
        self.channel_names = params.channel_names
        self.tracer_finetune = params.tracer_finetune
        # set a stream
        ...
        if params.tracer_finetune: 
            self.N_out_channels_tracer = params.N_tracer_out_channels
            if self.split_data_channels:
                self.out_channels_local_tracer = (params.N_tracer_out_channels + matmul_comm_size - 1) // matmul_comm_size
                mprank = comm.get_rank("matmul")
                self.clim = torch.split(self.clim, self.out_channels_local_tracer, dim=1)[mprank].contiguous()
            else:
                self.out_channels_local_tracer = params.N_tracer_out_channels
    ...
    def initialize_buffers(self):
        ...
        if self.tracer_finetune:
            self.acc_curve = torch.zeros((self.out_channels_local_tracer, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
            self.rmse_curve = torch.zeros((self.out_channels_local_tracer, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        ...
        if self.tracer_finetune:
            self.acc_curve_cpu = torch.zeros((self.out_channels_local_tracer, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
            self.acc_auc_cpu = torch.zeros((self.out_channels_local_tracer), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
            self.rmse_curve_cpu = torch.zeros((self.out_channels_local_tracer, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
        else:
    ...
    def update(self, prediction, target, loss, idt):
        ...
        if self.do_gather_input:
            prediction = self._gather_input(prediction.contiguous())
            target = self._gather_input(target.contiguous())
        # update parameters
