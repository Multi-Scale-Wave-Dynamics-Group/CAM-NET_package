...
def get_tracer_dataloader(params, physical_path, tracer_path, train=True,final_eval=False):
    from makani.utils.dataloader import init_distributed_io
    from makani.utils.dataloaders.data_loader_tracer_multifiles import TracerMultifilesDataset
    init_distributed_io(params)
    if params.get("multifiles", False):
        dataset = TracerMultifilesDataset(params, physical_path, tracer_path, train)
        sampler = None
        ...
        from makani.utils.dataloaders.data_loader_dali_2d_tracer import ERA5DaliESDataloadertracer as ERA5DaliESDataloader2Dtracer
        # dali loader
        dataloader = ERA5DaliESDataloader2Dtracer(params, physical_path, tracer_path, train, final_eval=final_eval)
        
        dataset = types.SimpleNamespace(
            in_channels=dataloader.in_channels,
            out_channels=dataloader.out_channels,
            tracer_in_channels = dataloader.tracer_in_channels,
            tracer_out_channels = dataloader.tracer_out_channels,
            grid_converter=dataloader.grid_converter,
            img_shape_x=dataloader.img_shape_x,
            img_shape_y=dataloader.img_shape_y,
            img_crop_shape_x=dataloader.img_crop_shape_x,
