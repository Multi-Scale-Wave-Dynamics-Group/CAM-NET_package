...
class Trainer:
    ...
    def _get_time_stats(self):
        ...
        if self.params.tracer_finetune is True:
            _, out_scale, _, out_scale_tracer = self.valid_dataloader.get_output_normalization()
            mult_cpu = torch.from_numpy(out_scale)[0, :, 0, 0]
            mult_cpu_tracer = torch.from_numpy(out_scale_tracer)[0, :, 0, 0]
        elif self.params.tracer_finetune is False:
        ...
        if self.params.enable_synthetic_data:
            ...
            # full bias and scale
            if self.params.tracer_finetune is True:
                in_bias, in_scale, in_bias_tracer, in_scale_tracer = self.valid_dataloader.get_input_normalization()
                in_bias = in_bias[0, ...]
                in_scale = in_scale[0, ...]
                in_bias_tracer = in_bias_tracer[0, ...]
                in_scale_tracer = in_scale_tracer[0, ...]
            elif self.params.tracer_finetune is False:
            ...
            if self.params.tracer_finetune is True:
                ...
                time_means_tracer = np.load(self.params.time_means_path_tracer)[0, self.params.tracer_out_channels, start_x:end_x, start_y:end_y]
                clim_tracer = torch.as_tensor((time_means_tracer - in_bias_tracer) / in_scale_tracer, dtype=torch.float32)
            elif self.params.tracer_finetune is False:
                time_means = np.load(self.params.time_means_path)[0, self.params.out_channels, start_x:end_x, start_y:end_y]
                clim = torch.as_tensor((time_means - in_bias) / in_scale, dtype=torch.float32)
        if self.params.tracer_finetune is True:
            return mult_cpu, clim, mult_cpu_tracer, clim_tracer
        elif self.params.tracer_finetune is False:
    ...
    def __init__(self, params, world_rank, job_type="train"):
        ...
        plot_list = [
            ...
            {
                "name": "zonalwind_u",
                "functor": "lambda x: x[0, ...]",
                "diverging": True
            },
            {
                "name": "meridionalwind_v",
                "functor": "lambda x: x[1, ...]",
                "diverging": True
            },
            {
                "name": "verticalwind_omega",
                "functor": "lambda x: x[3, ...]",
                "diverging": True
            },
            {
                "name": "temperature",
                "functor": "lambda x: x[4, ...]",
                "diverging": True
            }
                        ]
        ...
        self.iters = 0
        self.startEpoch = 0
        print(f"The resuming signal is {params.resuming}")
        print(f"The path is {params.checkpoint_path}")
    ...
    def train(self):
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            print(f"Checkpoint Path: {self.params.checkpoint_path}")
            ...
            #if self.params.save_checkpoint == "legacy":
                # checkpoint at the end of every epoch
            #    self.save_checkpoint(self.params.checkpoint_path, checkpoint_mode=self.params["save_checkpoint"])
            #    best_checkpoint_path = self.params.best_checkpoint_path.format(mp_rank=comm.get_rank("model"))
            #    best_checkpoint_saved = os.path.isfile(best_checkpoint_path)
            ...
            #        self.save_checkpoint(self.params.best_checkpoint_path, checkpoint_mode=self.params["save_checkpoint"])
            #        best_valid_loss = valid_logs["base"]["validation loss"]
            #elif (self.data_parallel_rank == 0) and (self.params.save_checkpoint == "flexible"):
            #    # checkpoint at the end of every epoch
            #    self.save_checkpoint(self.params.checkpoint_path, checkpoint_mode=self.params["save_checkpoint"])
            #    best_checkpoint_path = self.params.best_checkpoint_path.format(mp_rank=comm.get_rank("model"))
            #    best_checkpoint_saved = os.path.isfile(best_checkpoint_path)
        ...
        with torch.inference_mode():
            with torch.no_grad():
                ...
                for data in tqdm(self.valid_dataloader, desc="Validation progress", disable=not self.params.log_to_screen):
                    ...
                    inp, tar = self.preprocessor.cache_unpredicted_features(*gdata)
                    inp = self.preprocessor.flatten_history(inp)
                    print(f"the datashape of tar is {tar.shape}")
                    ...
                    for idt, targ in enumerate(tarlist):
                        ...
                        with amp.autocast(enabled=self.amp_enabled, dtype=self.amp_dtype):
                            ...
                            print(f"the datashape of inpt, tar, pred is {inpt.shape, targ.shape, pred.shape}")
    ...
    def save_checkpoint(self, checkpoint_path, model=None, checkpoint_mode="legacy"):
        """
        Save out checkpoint
        """
        ...
        with torch.no_grad():
            ...
            if checkpoint_mode == "legacy":
                ...
                print(f"the corresponding rank is {self.data_parallel_rank}") #{comm.get_rank('model')}")
                checkpoint_fname = checkpoint_path.format(mp_rank=self.data_parallel_rank) #comm.get_rank("model"))
                store_dict = {
                    "iters": self.iters,
                    "epoch": self.epoch,
    ...
    def restore_checkpoint(self, checkpoint_path, checkpoint_mode="legacy", load_optimizer=False, load_scheduler=False, load_counters=False):
        """
